...
class HeaderFile(Configurable):
    ...
    meta: MetaVars
    additional_header_files: list[HeaderFile]
    ...
    def __init__(self, config, structs, vals):
        ...
        self.path = self.config.out_dir / rf'{self.config.path.stem}.hpp'
        ...
        if not self.combined:
            ...
            self.path = self.config.out_dir / rf'{self.structs[0].name}.hpp'
        ...
        with SchemaContext('includes'):
            ...
            internal = set(self.includes.internal)
            external = set(self.includes.external)
            for struct in self.structs:
                internal.update(struct.includes.internal)
                external.update(struct.includes.external)
            self.includes.internal = sorted(internal)
            self.includes.external = sorted(external)
        ...
        # banner
        if self.banner:
            lines = [s.rstrip() for s in self.banner.split('\n')]
            lines = [re.sub(r'(^|\n)// ?', r'\1', s) for s in lines]
            lines = [rf'// {s}'.rstrip() for s in lines]
            self.banner = '\n'.join(lines)
